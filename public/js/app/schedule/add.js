requirejs(['jquery','webuploader','jquery_ui','dtree','reveal','formValidator','security'],function($,WebUploader,jquery_ui){
	...
	var fn = {
		...
		initLoginMebmer:function(current){ //初始化当前用户
			var str = '';
			str +='<div class="users">';
		    str +='<img src="'+current.avater+'" alt="用户图片"><p data-id="'+current.id+'">'+current.username+'</p>';
		    str +='<div class="del"></div>';
		    str += '</div>';
			$('.Mright').html(str);
		},
		getAllStaff:function(){	//获取部门和人员选择
			var that = this;
			$.ajax({
				type:'post',
				dataType:'json',
				...
				success:function(data){
					StaffData 	=	data.data;
					current 	= 	data.curruser;

					var str = '<span data-id="'+current.id+'"><em>'+current.username+'</em> <a id="userdel"></a></span>';
					$('.participantzone').html(str);

					$('.participantzone').on('click','#userdel',function(){
